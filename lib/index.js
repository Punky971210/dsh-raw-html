...
- 本机图片内联 + 一键下载：`<img src="/api/dsh-raw-html/file?path=<URL编码的绝对路径>">`，并在图旁给下载入口 `<a href="同URL" download="文件名">下载原图</a>`——该路由带鉴权、仅服务**会话工作区目录内**的文件、单文件上限 20 MiB；路径必须 URL 编码，超出白名单或超限会返回 403/413（图片不显示，属预期保护而非渲染故障）。
