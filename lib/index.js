...
- 本机图片内联 + 一键下载：用 <img src="/api/dsh-raw-html/file?path=<URL编码的绝对路径>"> 显示，图旁给下载入口 <a href="同URL" download="文件名">下载原图</a>——该路由带鉴权、仅服务**会话工作区目录内**的文件、单文件上限 20 MiB；路径必须 URL 编码，超出白名单或超限会返回 403/413（图片不显示，属预期保护而非渲染故障）。
...
function apply(ctx) {
  ...
  //  1) 通路：connection.fetch.register（exact Fetch route）——与宿主 /api/file
  //     完全同一机制；path 必须位于 /api 之下（宿主 assertFetchRoute 强制校验），
  //     因此自动享有 /api 前缀路由上的 Host/Origin fence 与浏览器 cookie 鉴权。
  //  2) 红线：绝不要用 webServer.register 裸注册同一路径——那条通路无鉴权
  //     （插件的 /fonts、/vendor 即为此形态），等于对外开放本机任意文件读取。
  ...
  //  5) 语义：默认 Content-Disposition: attachment（真下载，文件名 RFC 5987
  //     编码）；加 ?inline=1 改为 inline，便于预览。
  //  6) 扩展点：token / 核销 / 配额 / 上传口等待办，均挂在本路由一处即可。
  const FILE_ROUTE = '/api/dsh-raw-html/file'
  ...
  async function serveLocalFile(request) {
    ...
    return new Response(await fs.readFile(real), { headers })
  }

  /**
   * 注册本地文件路由。能力位前置：宿主未提供 connection.fetch.register 时
   * 静默跳过（不抛错、不影响插件其余功能）。
   * @returns 是否注册成功。
   */
  function installLocalFileRoute(connection) {
    if (connection === undefined || connection === null) return false
    const fetchFace = connection.fetch
    if (fetchFace === undefined || typeof fetchFace.register !== 'function') return false
    ctx.effect(() => fetchFace.register({
      path: FILE_ROUTE,
      methods: ['GET', 'HEAD'],
      requestBody: 'buffered',
      fetch: (request) => serveLocalFile(request),
  ...
  ctx.inject(['connection'], (connectionCtx) => {
    ...
    // 本地文件服务（带鉴权）：与 rpc 通道互相独立，缺 rpc 也要装。
    installLocalFileRoute(connection)
