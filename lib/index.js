...
/** 依赖的宿主服务（tools 用于注册 file_download_link，见「常驻能力 + 工具」段）。 */
const inject = ['settings', 'systemPrompt', 'webServer', 'tools']
...
- 本机图片内联 + 一键下载：用 <img src="/dsh-raw-file?path=<URL编码的绝对路径>"> 显示，图旁给下载入口 <a href="同URL" download="文件名">下载原图</a>——该路由带鉴权、仅服务**会话工作区目录内**的文件、单文件上限 20 MiB；路径必须 URL 编码，超出白名单或超限会返回 403/413（图片不显示，属预期保护而非渲染故障）。
...
async function apply(ctx) {
  // defineTool 用**动态** import 解析：宿主内核提供 @deepseek-ai/dsh-tools；解析不到时
  // 降级为「不注册工具」（常驻能力段与文件路由照常工作）。
  // 红线：**绝不用静态 import** —— 从 git 获取的分发形态没有 node_modules，
  // 静态 import 会让整个插件加载失败（`Cannot find package` ⇒ plugin tree failed to load）。
  let defineTool = null
  try {
  ...
  // === 本地文件服务（本地 fork 增补 · 2026-09-18）=============================
  // 目的：为「内联图片 + 一键下载」提供带鉴权的文件字节通道，替代已弃用的自建
  // 文件桥 dsh-file-portal。改动前请先读以下契约与红线：
  //  1) 通路：本插件自己的 webServer 前缀路由（与 /fonts、/vendor 同处一个 effect，
  //     生命周期随插件，稳定）。**不要**改用 connection.fetch.register —— 实测该
  //     通路注册的路由在启动后会被 effect 清理（注册时命中=true，5 秒后即消失），
  //     第三方插件晚注册尤其如此；宿主自己的 /api/file 由内核编排注册才稳。
  //  2) 鉴权：在 handler 内复用宿主 connection 服务的 requestRejection(req)，
  //     与 /api 前缀路由同一套 Host/Origin fence + 浏览器 cookie 校验；
  //     无法取得该服务时一律 401（安全默认），绝不裸放行。
  //  3) 白名单：宿主已注册的各会话工作区目录（ctx.workspaceRegistry.list()），
  //     经 realpath 归一后做前缀比对，防 ".." 与符号链接逃逸；不在白名单内 403。
  //  4) 上限：20 MiB，与宿主 attachments.imageLimits.maxImageBytes 默认值对齐。
  //  5) 语义：默认 Content-Disposition: attachment（真下载，文件名走 RFC 5987
  //     编码）；加 ?inline=1 改为 inline，便于预览。
  //  6) 扩展点：token / 核销 / 配额 / 上传口等待办，均挂在本路由一处即可。
  const FILE_ROUTE = '/dsh-raw-file'
  ...
  const FILE_MIME = {
    ...
    '.json': 'application/json', '.csv': 'text/csv; charset=utf-8', '.zip': 'application/zip',
    '.mp4': 'video/mp4', '.mp3': 'audio/mpeg',
  }

  /** Content-Disposition 的 ASCII 回退名（非 ASCII 以 _ 代；真名走 RFC 5987 参数）。 */
  function fileAsciiName(value) {
    return String(value).replace(/[^\x20-\x7E]/g, '_').replace(/["\\]/g, '_')
  }

  /** 白名单根：各会话工作区目录（realpath 归一；已不存在的目录静默跳过）。 */
  async function fileAllowedRoots() {
    const out = []
    let records = []
    try {
      const registry = ctx.get('workspaceRegistry') || ctx.workspaceRegistry
      if (registry && typeof registry.list === 'function') records = registry.list() || []
    } catch {
      records = []
    }
    for (const record of records) {
      const p = record && typeof record.path === 'string' ? record.path : ''
      if (p === '') continue
      try {
        out.push(await fs.realpath(p))
      } catch {
        /* 目录不存在或不可读：跳过，不影响其他根 */
      }
    }
  ...
  // === 路径白名单契约（迁移自已弃用插件 dsh-file-portal 的 core/paths.ts · 2026-09-18）===
  // 契约要点（改动前必读；顺序不得跳过、判定不得降级）：
  //  1) fail-closed 校验顺序：空 → NUL → `..` 段 → UNC/设备路径 → 相对则锚定工作区 →
  //     两侧 realpath → **按路径段判定包含** → 拒非普通文件；
  //  2) **禁止用 startsWith 做包含判定**：它会接受同名前缀的兄弟目录（`<root>-evil/`），
  //     文件桥把它登记为 FORBIDDEN_WEAK_CONTAINMENT；必须用 path.relative() 做段判定；
  //  3) forbiddenRoot（DSH home）下的凭据/密钥/配置**一律拒绝**，即使某个白名单根命中也不放行。
  const PARENT_SEGMENT = /(?:^|[\\/])\.\.(?:[\\/]|$)/
  const DEVICE_PATH = /^[\\/]{1,2}[.?][\\/]/
  const UNC_PATH = /^[\\/]{2}[^\\/]/
  const FILE_FORBIDDEN_ROOT = path.join(os.homedir(), '.dsh')

  /** 两路径是否指向同一位置（Windows 大小写不敏感；两侧都应是 realpath 结果）。 */
  function fileSamePath(left, right) {
    return process.platform === 'win32' ? left.toLowerCase() === right.toLowerCase() : left === right
  }

  /** child 是否位于 root 之内——**按路径段**判定（禁止 startsWith 前缀判定）。 */
  function fileContainedIn(root, child) {
    if (fileSamePath(root, child)) return true
    const rel = path.relative(root, child)
    if (rel === '' || rel.startsWith('..')) return false
    return !path.isAbsolute(rel)
  }

  /** 本地文件响应（node req/res）：鉴权 → 白名单 → 上限 → 下载语义。 */
  async function serveLocalFile(req, res, connection) {
    const base = { 'Cache-Control': 'private, no-store', 'X-Content-Type-Options': 'nosniff' }
    const fail = (status, text) => {
      res.writeHead(status, { ...base, 'Content-Type': 'text/plain; charset=utf-8' })
      res.end(req.method === 'HEAD' ? undefined : text)
    }
    // 鉴权：与 /api 前缀路由同一套（Host/Origin fence + 浏览器 cookie）。
    // 拿不到 connection 服务时一律 401 —— 安全默认，绝不裸放行文件读取。
    let rejection = 401
    try {
    ...
    if (rejection !== undefined) {
      res.writeHead(rejection, { ...base, 'Content-Type': 'text/plain; charset=utf-8' })
    ...
    try {
      ...
      wantsInline = url.searchParams.get('inline') === '1'
    } catch {
      return fail(400, 'bad request')
    }
    // —— 路径白名单契约（fail-closed；顺序见上方契约注释，不得跳过）——
    if (requested.trim() === '') return fail(400, 'missing path')
    if (requested.includes('\u0000')) return fail(400, 'bad path')
    if (PARENT_SEGMENT.test(requested)) return fail(403, 'parent traversal is refused')
    if (UNC_PATH.test(requested) || DEVICE_PATH.test(requested)) return fail(403, 'UNC or device path is refused')
    const roots = await fileAllowedRoots()
    if (roots.length === 0) return fail(403, 'this host knows no session workspace directory')
    // 相对候选按契约锚定到会话工作区（roots[0]），随后走同一套包含判定——绝不凭信任放行。
    const anchored = path.isAbsolute(requested) ? requested : path.resolve(roots[0], requested)
    let real
    try {
      real = await fs.realpath(anchored)
    } catch (error) {
      const code = error && error.code
      return fail(code === 'ENOENT' || code === 'ENOTDIR' ? 404 : 403, 'not found')
    }
    // forbiddenRoot 优先：DSH home 下的凭据/密钥/配置一律拒绝（即使某个白名单根命中）。
    let forbiddenReal = null
    try {
      forbiddenReal = await fs.realpath(FILE_FORBIDDEN_ROOT)
    } catch {
      forbiddenReal = null
    }
    if (forbiddenReal !== null && fileContainedIn(forbiddenReal, real)) {
      return fail(403, 'the harness home is never deliverable')
    }
    // 包含判定：按路径段比较（两侧均为 realpath 结果）；禁止 startsWith 前缀判定。
    if (!roots.some((root) => fileContainedIn(root, real))) {
      return fail(403, 'outside the session workspaces')
    }
    let info
    try {
    ...
    if (!info.isFile()) return fail(403, 'not a regular file')
    if (info.size > FILE_MAX_BYTES) return fail(413, 'file exceeds ' + FILE_MAX_BYTES + ' bytes')
    const downloadName = path.basename(real)
    const headers = {
      ...base,
      'Content-Type': FILE_MIME[path.extname(real).toLowerCase()] || 'application/octet-stream',
      'Content-Length': String(info.size),
      'Content-Disposition':
        (wantsInline ? 'inline' : 'attachment') +
        '; filename="' + fileAsciiName(downloadName) + '"' +
  ...
  /**
   * 注册本地文件路由（webServer 前缀路由，与 /fonts、/vendor 同处一个生命周期）。
   * 鉴权在 handler 内做：请求时动态取 connection 服务（不依赖注入时机），
   * 拿不到则一律 401。
   * @returns webServer 注册返回的 disposer。
   */
  function installLocalFileRoute(webServer) {
    return webServer.register({
      kind: 'prefix',
      path: FILE_ROUTE,
      handler: (req, res) => serveLocalFile(req, res, ctx.get('connection') || ctx.connection),
    })
  }

  // 注册：本 effect 与插件同生命周期（注：曾试过 connection.fetch.register 通路，
  // 实测其注册会被 effect 在启动后清理，故改用插件自有 webServer 路由 + handler 内鉴权）。
  ctx.effect(() => {
    if (webServer === undefined || webServer === null || typeof webServer.register !== 'function') {
      console.log('[raw-html] local file route 未注册：webServer 服务不可用')
      return undefined
    }
    const disposer = installLocalFileRoute(webServer)
    console.log('[raw-html] local file route 已注册: ' + FILE_ROUTE)
    return disposer
  }, 'raw-html: local file route')

  // === 常驻能力说明 + file_download_link 工具（可发现性 · 本地 fork 增补）=========
  // 背景：下载能力的说明原本写在 VCP 协议段里，而那段以 render 开关为条件
  // （`text: () => render ? 协议全文 : DISABLED_TEXT`）⇒ 关掉渲染就无人知晓
  // （实测：render 默认 false、状态文件不存在 ⇒ 所有会话都看不到该能力）。故拆两条：
  //   ① 常驻 systemPrompt.section（不受 render 约束）——让每个会话的 agent 知道有这条路；
  //   ② file_download_link 工具——工具列表天然可见，且返回**纯文本 URL**，
  //      不依赖 HTML 渲染开关，agent 可主动调用、用户直接粘贴到浏览器。
  const FILE_CAPABILITY_TEXT = `【本机文件下载能力（dsh-raw-html 提供）】
需要让用户取走工作区内的文件（图片/文档/任意类型）时，二选一：
1) 调工具 file_download_link（入参 path=文件绝对路径）取得下载链接——推荐，且不依赖 HTML 渲染开关；
2) 在回复里写卡片：<img src="/dsh-raw-file?path=<URL编码的绝对路径>">，图旁配 <a href="同URL" download="文件名">下载</a>（此路需 HTML 渲染开关开启）。
路由 /dsh-raw-file 只服务宿主已注册的会话工作区目录内文件、单文件 ≤ 20 MiB；越界 403、超限 413、无浏览器凭据 401。`

  ctx.effect(() => systemPrompt.section({
    name: 'raw-html:file-download',
    order: 210,
    text: () => FILE_CAPABILITY_TEXT,
  }), 'raw-html: file capability section')

  const FILE_LINK_TOOL = defineTool === null ? null : defineTool({
    name: 'file_download_link',
    description: '把工作区内的文件转成可下载链接（返回可粘贴的 URL）。仅当路径位于宿主已注册的会话工作区内、且文件不超过 20 MiB 时可用；不依赖 HTML 渲染开关。',
    parameters: {
      path: { type: 'string', required: true, description: '要交付的文件绝对路径（Windows 盘符或 POSIX 形式）' },
      inline: { type: 'boolean', description: '返回内联预览链接（?inline=1；仅图片/PDF 等浏览器能直接显示的类型有意义）' },
      origin: { type: 'string', description: '可选：GUI 源地址（如 http://127.0.0.1:3080）；给出则返回绝对 URL，省略则返回相对路径' },
    },
    output: {
      schema: {
        type: 'object',
    ...
    async execute(args) {
      const empty = (message) => ({ ok: false, message, url: '', name: '', size: 0, mime: '' })
      const requested = typeof args?.path === 'string' ? args.path.trim() : ''
      if (requested === '') return empty('缺少 path 参数（需要文件的绝对路径）')
      if (!path.isAbsolute(requested)) return empty('path 必须是绝对路径：' + requested)
      let real
      try {
        real = await fs.realpath(path.resolve(requested))
      } catch {
        return empty('文件不存在或不可读：' + requested)
      }
      const roots = await fileAllowedRoots()
      if (roots.length === 0) return empty('宿主尚未注册任何会话工作区，无法判定白名单（路由会返回 403）')
      // 与路由同一套判定：forbiddenRoot 优先 + 按路径段包含（禁止 startsWith 前缀判定），
      // 否则会出现「工具说可以、路由返回 403」的不一致。
      let forbiddenReal = null
      try {
        forbiddenReal = await fs.realpath(FILE_FORBIDDEN_ROOT)
      } catch {
      ...
      if (forbiddenReal !== null && fileContainedIn(forbiddenReal, real)) {
        return empty('该路径位于宿主 home（~/.dsh）之下，凭据/密钥/配置禁止交付（路由会返回 403）：' + real)
      }
      if (!roots.some((root) => fileContainedIn(root, real))) {
        return empty('该路径不在会话工作区内，路由会返回 403：' + real)
      }
      let info
      try {
        info = await fs.stat(real)
      } catch {
        return empty('无法读取文件状态：' + real)
      }
      if (!info.isFile()) return empty('不是普通文件（目录暂不支持打包下载）：' + real)
      if (info.size > FILE_MAX_BYTES) {
        return empty('文件 ' + info.size + ' B 超过 ' + FILE_MAX_BYTES + ' B 上限，路由会返回 413：' + real)
      }
      const fileName = path.basename(real)
      const mime = FILE_MIME[path.extname(real).toLowerCase()] || 'application/octet-stream'
      const relative = FILE_ROUTE + '?path=' + encodeURIComponent(real) + (args?.inline === true ? '&inline=1' : '')
      const rawOrigin = typeof args?.origin === 'string' ? args.origin.trim() : ''
      const url = rawOrigin === '' ? relative : rawOrigin.replace(/\/+$/, '') + relative
      const lines = [
        '文件：' + fileName + '（' + info.size + ' B · ' + mime + '）',
        '下载链接：' + url,
      ]
      if (rawOrigin === '') {
        lines.push('（相对链接：完整地址 = 用户访问 GUI 的地址 + 上述路径，例如 http://127.0.0.1:3080' + relative + '）')
      }
      lines.push('链接仅在已登录的浏览器内有效（无 cookie 访问返回 401）；外部下载器因缺凭据当前不支持。')
      return { ok: true, message: lines.join('\n'), url, name: fileName, size: info.size, mime }
    },
  })

  ctx.effect(() => {
    if (tools === undefined || tools === null || typeof tools.register !== 'function') {
      console.log('[raw-html] file_download_link 未注册：tools 服务不可用')
      return undefined
    }
    if (FILE_LINK_TOOL === null) {
      console.log('[raw-html] file_download_link 未注册：@deepseek-ai/dsh-tools 不可解析（常驻能力段仍生效）')
      return undefined
    }
    const disposer = tools.register(FILE_LINK_TOOL)
    console.log('[raw-html] file_download_link 工具已注册')
    return disposer
  }, 'raw-html: file_download_link tool')
